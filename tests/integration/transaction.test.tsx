...
describe('Transaction Flow Integration', () => {
    ...
    it('should complete full transaction flow from send to confirmation', async () => {
        ...
        const mockSignTransaction =
            require('@/services/solana.service').signTransaction;
        const mockSendTransaction =
            require('@/services/solana.service').sendTransaction;
        ...
        mockSignTransaction.mockResolvedValue({
            success: true,
            signedTransaction: 'signed_tx_data',
        });
        ...
        await waitFor(() => {
            expect(
                screen.getByText(/confirm transaction/i)
            ).toBeInTheDocument();
            expect(
                screen.getByText(/recipient: jane smith/i)
            ).toBeInTheDocument();
        ...
        await waitFor(() => {
            expect(
                screen.getByText(/transaction sent successfully/i)
            ).toBeInTheDocument();
            expect(
                screen.getByText(/transaction id: tx123/i)
            ).toBeInTheDocument();
        });
    ...
    it('should handle transaction with contact selection', async () => {
        const mockGetContacts =
            require('@/services/contact.service').getContacts;
        const mockSendMoney =
            require('@/services/transaction.service').sendMoney;
        const mockValidateRecipient =
            require('@/services/transaction.service').validateRecipient;
    ...
    it('should save transaction to contact when option is selected', async () => {
        ...
        const recipientInput = screen.getByLabelText(/recipient/i);
        const amountInput = screen.getByLabelText(/amount/i);
        const saveToContactsCheckbox =
            screen.getByLabelText(/save to contacts/i);
        const sendButton = screen.getByRole('button', { name: /send/i });
    ...
    it('should show transaction history after successful transaction', async () => {
        ...
        const mockGetTransactionHistory =
            require('@/services/transaction.service').getTransactionHistory;
        ...
        const viewHistoryButton = screen.getByRole('button', {
            name: /view transaction history/i,
        });
