...
describe('ContactList', () => {
    ...
    it('should show empty state when no contacts exist', () => {
        ...
        expect(screen.getByText(/add your first contact to get started/i)).toBeInTheDocument()
    })
    ...
    it('should add new contact successfully', async () => {
        ...
        mockAddContact.mockResolvedValue({ success: true, contact: { id: '4', name: 'New Contact' } })
    ...
    it('should toggle favorite status when star is clicked', async () => {
        const mockUpdateContact = require('@/services/contact.service').updateContact
        ...
        await waitFor(() => {
            expect(mockUpdateContact).toHaveBeenCalledWith('2', { isFavorite: true })
        })
    })
    ...
    it('should update contact successfully', async () => {
        ...
        fireEvent.change(nameInput, { target: { value: 'Jane Smith Updated' } })
        fireEvent.click(saveButton)
    ...
    it('should delete contact when delete button is clicked', async () => {
        const mockDeleteContact = require('@/services/contact.service').deleteContact
        ...
        expect(screen.getByText(/are you sure you want to delete this contact/i)).toBeInTheDocument()
        ...
        await waitFor(() => {
            ...
            expect(screen.getByText(/contact deleted successfully/i)).toBeInTheDocument()
        })
    })
    ...
    it('should send money to contact when send money button is clicked', () => {
        ...
        const sendButtons = screen.getAllByRole('button', { name: /send money/i })
        fireEvent.click(sendButtons[0])

        expect(mockPush).toHaveBeenCalledWith('/send?recipient=jane@example.com')
    })

    it('should show last transaction date', () => {
        render(<ContactList />)

        expect(screen.getByText(/last sent: jan 15, 2024/i)).toBeInTheDocument()
        expect(screen.getByText(/last sent: jan 14, 2024/i)).toBeInTheDocument()
        expect(screen.getByText(/last sent: jan 13, 2024/i)).toBeInTheDocument()
    })
    ...
    it('should handle contact service errors gracefully', async () => {
        const mockAddContact = require('@/services/contact.service').addContact
        mockAddContact.mockResolvedValue({ success: false, error: 'Contact already exists' })
        ...
        await waitFor(() => {
            expect(screen.getByText(/contact already exists/i)).toBeInTheDocument()
        })
    })
})
