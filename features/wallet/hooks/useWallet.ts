...
interface UseWalletReturn {
    ...
    swapCurrencies: (fromCurrency: string, toCurrency: string, amount: number) => Promise<void>
    getTransactionFees: (amount: number, currency: string) => Promise<TransactionFee>
    clearError: () => void
}
...
export const useWallet = (userId: string): UseWalletReturn => {
     ...
     * Disconnect wallet
     */
    const disconnectWallet = useCallback(async (walletAddress: string) => {
        try {
            setIsLoading(true)
            setError(null)
     ...
     * Swap currencies
     */
    const swapCurrencies = useCallback(async (fromCurrency: string, toCurrency: string, amount: number) => {
        try {
            setIsLoading(true)
            setError(null)
            
            await WalletService.swapCurrencies(userId, fromCurrency, toCurrency, amount)
     ...
     */
    const getTransactionFees = useCallback(async (amount: number, currency: string): Promise<TransactionFee> => {
        try {
